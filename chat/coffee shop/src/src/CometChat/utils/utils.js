...
// Initializes CometChat localization
export function setupLocalization(language) {
  const resourcesJson = {
    'en-US': englishUS,
    'en-UK': englishUK,
    'ru-RU': russian,
...
function blendColorWith(color, percentage, blendWith) {
...
export function generateExtendedColors() {
  ...
  if (primaryColor) {
    ...
    percentages.forEach((percentage, index) => {
      const color = (index === percentages.length - 1) ? lastBlendColor : blendColor;

      if (index === 0) extendedVar = 50;
      else if (index === 1) extendedVar = 100;
      else extendedVar += 100;
